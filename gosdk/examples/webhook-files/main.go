...
func main() {
	...
	fmt.Println("✓ Multiple files sent")
	...
	fmt.Println("✓ Log file with embed sent")
...
func sendMultipleFiles(ctx context.Context, client *webhook.Client) error {
  ...
  "timestamp": "2025-11-08T12:00:00Z"
	...
	msg := &types.WebhookMessage{
		Content: "📁 Uploading multiple files:",
	}
...
`
	...
	now := time.Now()
	msg := &types.WebhookMessage{
		Content: "📋 Application logs attached",
		Embeds: []types.Embed{
			{
				...
				Fields: []types.EmbedField{
					{
						Name:   "Level",
						Value:  "⚠️ Contains warnings and errors",
						Inline: true,
					},
					{
						Name:   "Lines",
					...
					{
						Name:   "Issues Found",
						Value:  "• High memory usage\n• API connection failure (resolved)",
						Inline: false,
					},
				},
...
func sendImage(ctx context.Context, client *webhook.Client) error {
	...
	msg := &types.WebhookMessage{
		Content: "🖼️ Image upload example",
	}
